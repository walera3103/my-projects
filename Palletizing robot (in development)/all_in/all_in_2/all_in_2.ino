...
int target_points[] = { target_1_point, target_2_point };
bool motor_moving[2] = { false, false };
int motor_direction[2] = { 0, 0 }; // 1 = forward, -1 = backward, 0 = stop
...
void setup() {
  ...
  Serial.println("System initialized - Simple stable control");
}
  ...
  if (now - lastControlUpdate >= 20) {
    for (int i = 0; i < 2; i++) {
      ...
      if (motor_moving[i]) {
        ...
        // Якщо не зупинились - продовжуємо в тому ж напрямку
        else {
          // Просто продовжуємо рух в тому ж напрямку
          // Не змінюємо швидкість постійно
        }
      } else {
