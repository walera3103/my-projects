...
# --- Control variables ---
MAX_VALUE = 40950
motor_targets = [20000, 20000, 20000, 20000, 20000]  # 5 моторів
encoder_values = [0, 0, 0, 0, 0]
lock = threading.Lock()
running = True

# Змінні для утримання кнопок
button_pressed = [None, None]  # [motor_index, direction]
...
# --- GUI ---
root = tk.Tk()
root.title("5 Motors Control with Hold Buttons")
...
def create_motor_controls(motor_number, row):
    ...
    title_frame.grid(row=0, column=0, columnspan=5, pady=(0, 8))
    tk.Label(title_frame, text=f"MOTOR {motor_number}",
             font=("Arial", 11, "bold")).pack()

    # Кнопка "-"
    minus_btn = tk.Button(frame, text="-", font=button_font, width=3, height=1,
                          bg="lightcoral")
    minus_btn.grid(row=1, column=0, padx=3)

    # Повзунок
    slider = ttk.Scale(frame, from_=0, to=MAX_VALUE, orient=tk.HORIZONTAL,
                       length=200)
    slider.set(motor_targets[motor_number - 1])
    slider.grid(row=1, column=1, padx=8, sticky="ew")
    sliders.append(slider)

    # Кнопка "+"
    plus_btn = tk.Button(frame, text="+", font=button_font, width=3, height=1,
                         bg="lightgreen")
    plus_btn.grid(row=1, column=2, padx=3)
    ...
    value_label.grid(row=1, column=3, padx=8)
    value_labels.append(value_label)

    # Лейбл з значенням енкодера
    encoder_label = tk.Label(frame, text="Encoder: 0",
                             font=("Arial", 9), fg="green")
    encoder_label.grid(row=2, column=0, columnspan=4, pady=(6, 0))
    ...
    tk.Label(range_frame, text="0", font=("Arial", 7)).pack(side=tk.LEFT)
    tk.Label(range_frame, text=f"{MAX_VALUE}", font=("Arial", 7)).pack(side=tk.RIGHT)
...
instruction.grid(row=5, column=0, columnspan=4)
